...
function doAccessibilitySettings() {
	...
	if (!localStorage.prefersSwitchCheckboxes) {
		...
		extraStyles += String.raw`input[type="checkbox"] {
  width: 6em !important;
	height: 3em !important;
	background: linear-gradient(to bottom, var(--backgroundBottom), var(--backgroundTop)) !important;
}
...
input[type="checkbox"]:before {
  content: " ";
	...
	background: linear-gradient(to bottom, var(--backgroundTop), var(--backgroundBottom)) !important;
	opacity: 1 !important;
	position: relative !important;
	left: 0.25em !important;
	transform: none !important;
	border: var(--borderWidth) var(--border) solid !important;
	border-radius: var(--borderRadius);
	box-shadow: 0 1px 2px #0008;
	
	transition: var(--animationDuration) var(--animationFunction, ease);
}

input[type="checkbox"]:checked {
  background: linear-gradient(to bottom, var(--accent), var(--accentHilight)) !important;
}
